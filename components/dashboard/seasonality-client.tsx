...
export function SeasonalityClient({ data }: SeasonalityClientProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Sazonalidade</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-1">Produtos em safra por mês e região</p>
      </div>

      {/* Current month highlight */}
      <Card className="border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/30">
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold text-green-800 dark:text-green-300">
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {currentMonthProducts.length === 0 ? (
              <p className="text-sm text-green-700 dark:text-green-400">Nenhum produto cadastrado para este mês.</p>
            ) : (
        ...
        <CardContent>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr>
                  <th className="text-left py-2 pr-4 font-medium text-gray-700 dark:text-gray-300 w-32">Produto</th>
                  {MONTHS.map((month) => (
                    <th
                      key={month}
                      className={cn(
                        'text-center py-2 px-1 font-medium w-10 text-xs',
                        month === currentMonth ? 'text-green-700 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'
                      )}
                    >
              ...
              <tbody>
                {regionData.map((item) => (
                  <tr key={item.id} className="border-t border-gray-100 dark:border-gray-700">
                    <td className="py-2 pr-4 font-medium text-gray-900 dark:text-gray-100">{item.product}</td>
                    {MONTHS.map((month) => (
                      <td key={month} className={cn('text-center py-2 px-1', month === currentMonth ? 'bg-green-50 dark:bg-green-900/20' : '')}>
                        {item.idealMonths.includes(month) ? (
                          ...
                          <span className="inline-block w-5 h-5 rounded-full bg-gray-100 dark:bg-gray-700" />
                        )}
                      </td>
                    ))}
                  </tr>
                ...
                {regionData.length === 0 && (
                  <tr>
                    <td colSpan={13} className="text-center py-8 text-gray-500">
                      <span className="text-gray-500 dark:text-gray-400">Nenhum produto cadastrado para {selectedRegion}</span>
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      <Card>
        ...
        <CardContent>
          ...
          {monthData.length === 0 ? (
            <p className="text-sm text-gray-500 dark:text-gray-400">Nenhum produto cadastrado para {getMonthName(selectedMonth)}.</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {monthData.map((item) => (
                <div key={item.id} className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  ...
                  <div>
                    <p className="font-medium text-gray-900 dark:text-gray-100 text-sm">{item.product}</p>
